...
- Enhanced Data Access Object for the EpubCoverEntity with offline support
  */
  ...
  @Query("SELECT * FROM epub_covers WHERE " + "(:showEpubs = 1 AND mimeType = ‘application/epub+zip’) OR " +  "(:showPdfs = 1 AND mimeType = ‘application/pdf’)")
  List<EpubCoverEntity> getCoversByFileTypes(boolean showEpubs, boolean showPdfs);
  
  // Get covers by books-only filter
  @Query("SELECT * FROM epub_covers WHERE " + "((:booksOnly = 1 AND isBook = 1) OR (:booksOnly = 0)) AND " + "((:showEpubs = 1 AND mimeType = ‘application/epub+zip’) OR " +  "(:showPdfs = 1 AND mimeType = ‘application/pdf’))")
  ...
  @Query("SELECT * FROM epub_covers WHERE " +  "(collectionKeys = ‘’ OR collectionKeys IS NULL OR collectionKeys LIKE ‘%’ || :collectionKey || ‘%’) AND " +  "((:showEpubs = 1 AND mimeType = ‘application/epub+zip’) OR " +"(:showPdfs = 1 AND mimeType = ‘application/pdf’)) AND " + "((:booksOnly = 1 AND isBook = 1) OR (:booksOnly = 0))")
